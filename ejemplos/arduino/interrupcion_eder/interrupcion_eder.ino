 ...
 * LGA, 25/06/2018
 * EGG, 28/06/2018 Se agrega conexion al relay
 * EDR, 28/06/2018 Trabajando con las interrupciones.
 * 
 * ***********************************************************
 * ******* OJO QUE NO RESPETA LOS FINALES DE CARRERA *********
 * ***********************************************************
 */
...
// Declaracion Variables
  ...
  boolean sentido = HIGH; //high: ida LOW vuelta

  boolean interrupcion = LOW;
...
void loop() {
  ...
  while(interrupcion == HIGH) {
    digitalWrite(Pin_enable,LOW);
    if(sentido == adelante) {
      detachInterrupt(0); //INT=0 -> Interrupción externa = Pin2; INT=1 -> Interrupción externa = Pin3
      mover(1, 100000, atras, 200);
      attachInterrupt(0, limiteX, LOW); //INT=0 -> Interrupción externa = Pin2; INT=1 -> Interrupción externa = Pin3
    }
    if(sentido == atras) {
      mover(1, 10, adelante, 200);
      interrupcion = LOW;
